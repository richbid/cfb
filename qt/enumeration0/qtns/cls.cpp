...
inline bool qtns::cls::compare_to(cls& aValue) const {
	if (mPMySize != aValue.getPMySize()) return false;
	return true;
}
...
qtns::cls::cls(const qtns::cls::MySize& aPMySize){
	setPMySize(aPMySize);
}
...
const QString field_keys[] = { "PMySize",  };
...
int qtns::cls::input_field(const QString& key, const QVariant& value, bool _set_value) {
	...
	if (!key.length()) return 0;
		...
		if (_set_value) {
			qtns::cls::MySize v = (qtns::cls::MySize)value.toInt();
			setPMySize(v);
		}
		result = 1;
	}
...
int qtns::cls::to_json(QString& output) {
	output = QString("{") + "\"PMySize\"" + ":" + "\"" + to_string((int)getPMySize()) + "\"" + "}";
	return 1;
}
...
int qtns::cls::to_xml(QString& output, const QString& rowTag) {
	output = "<" + rowTag + ">""<PMySize>" + to_xml_string(to_string(getPMySize())) + "</PMySize>" + "</" + rowTag + ">";
	return 1;
}
