...
int ns::cls2::input_field(const QString& key, const QVariant& value, bool _set_value) {
	int result = 0;
...
int ns::cls2::to_json(QString& output) {
	output = QString("{") + "\"PInt\"" + ":" + "\"" + to_string(getPInt()) + "\"" + "}";
	return 1;
}
...
int ns::cls2::to_xml(QString& output, const QString& rowTag) {
	output = "<" + rowTag + ">""<PInt>" + to_xml_string(to_string(getPInt())) + "</PInt>" + "</" + rowTag + ">";
	return 1;
}
...
int ns::cls2::to_sv(QString& output, const QString& spliter) {
	if (!spliter.length()) return 0;
	QStringList list;
	list << to_string(getPInt());
