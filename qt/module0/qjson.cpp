...
QVariant QJson::decode(const QString& jsonString) {
	...
	if (!token.isEmpty())
		result = QObject::tr("Invalid JSON string -- remaining token %1 at position %2.").arg(token).arg(position);
	
	return result;
}

QString QJson::nextToken(const QString& jsonString, int& position) {
...
QString QJson::peekToken(const QString& jsonString, int position) {
	return (nextToken(jsonString, position));
}

QVariant QJson::parseInternal(const QString& jsonString, int& position, int nesting) {
...
QVariant QJson::parseObject(const QString& jsonString, int& position, int nesting) {
...
QVariant QJson::parseArray(const QString& jsonString, int& position, int nesting) {
...
QVariant QJson::parseString(const QString& jsonString, int& position, int nesting) {
	QString result;
...
QVariant QJson::parseBool(const QString& jsonString, int& position, int nesting) {
	bool resultBool;
	QString token;
...
QVariant QJson::parseNull(const QString& jsonString, int& position, int nesting) {
	QString token;
	token = nextToken(jsonString, position);
	Q_ASSERT(token == "null");
	Q_UNUSED(token);
...
QVariant QJson::parseNumber(const QString& jsonString, int& position, int nesting) {
...
QVariant QJson::syntaxError(const QString& method, const QString& jsonString, int position, int nesting, Error error) {
	QString text;
	switch (error) {
	...
	QString errorMsg = QObject::tr("*** Error %1 in QJson::%2 at position %3 -- %4").arg(error).arg(method).arg(position).arg(text);
	QStringList errorList;
	errorList << errorMsg;
	return QVariant(errorList);
}

QString QJson::encode(const QVariant& jsonObject) {
	...
	default:
		return encodingError("encode", jsonObject, ErrorUnrecognizedObject);
	}
}

QString QJson::encodeObject(const QVariant& jsonObject) {
	QString result("{ ");
...
QString QJson::encodeArray(const QVariant& jsonObject) {
	QString result("[ ");
	QList<QVariant> list = jsonObject.toList();
	for (int i = 0; i < list.count(); i++) {
		result.append(encode(list.at(i)));
		if (i+1 < list.count()) {
			result.append(", ");
		}
	}
...
QString QJson::encodeString(const QVariant &jsonObject) {
	return encodeString(jsonObject.toString());
}

QString QJson::encodeString(const QString& value) {
	QString result = "\"";
...
QString QJson::encodeNumeric(const QVariant& jsonObject) {
	return jsonObject.toString();
}

QString QJson::encodeBool(const QVariant& jsonObject) {
	return jsonObject.toString();
}

QString QJson::encodeNull(const QVariant& jsonObject) {
	return "null";
}

QString QJson::encodingError(const QString& method, const QVariant& jsonObject, Error error) {
	QString text;
	switch (error) {
	...
	return QObject::tr("*** Error %1 in QJson::%2 -- %3").arg(error).arg(method).arg(text);
}
